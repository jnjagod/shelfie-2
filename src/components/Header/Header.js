import React, { Component } from 'react'
import { Link } from 'react-router-dom'
import './Header.css'

export default class Header extends Component {
  render() {
    return (
      <header>
        <img className='logo' src="data:image/png;base64," alt="" />
        <h1>SHELFIE</h1>
        <div className="header-nav">
          <Link to='/'><button style={{marginLeft: '50px'}} className='red-button'>Dashboard</button></Link>
          <Link to='/add'><button style={{marginLeft: '50px'}} className='red-button'>Add Inventory</button></Link>
        </div>
      </header>
    )
  }
}